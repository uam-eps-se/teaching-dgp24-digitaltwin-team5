...
const actions = (roomId: number) => [
  {
    icon: <Icon path={mdiTrashCan} size={1} color='var(--mui-palette-error-main)' />,
    name: 'Delete Room'
  },
  ...
  return (
    <div>
      ...
      >
          ...
          >
            {actions(room.id).map(action => (
              <SpeedDialAction
                ...
                onClick={() => {
                  if (action.path) router.push(`/rooms/${room.id}/edit`)
        ...
        <DeleteRoomModal roomId={room.id} roomName={room.name} open={openDelete} setOpen={setOpenDelete} />
      </Box>
    </div>
  )
}
